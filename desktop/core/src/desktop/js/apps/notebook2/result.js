...
class Result {
  constructor(snippet, result) {
    ...
    self.data = ko.observableArray(result.data || []);
    self.data.extend({ rateLimit: 50 });
    self.explanation = ko.observable(result.explanation || '');
    self.images = ko.observableArray(result.images || []);
    self.images.extend({ rateLimit: 50 });
    self.logs = ko.observable('');
    self.logLines = 0;
    self.hasSomeResults = ko.pureComputed(() => self.hasResultset() && self.data().length > 0); // status() == 'available'
  }

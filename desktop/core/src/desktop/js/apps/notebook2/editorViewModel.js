...
import ChartTransformers from 'apps/notebook2/chartTransformers';
...
class EditorViewModel {
  constructor(editor_id, notebooks, options, CoordinatorEditorViewModel, RunningCoordinatorModel) {
    ...
    self.editorTypeTitle = ko.pureComputed(() => {
      ...
      return foundInterpreter ? foundInterpreter.name : self.editorType();
    });
    ...
    const sqlSourceTypes = self.sqlSourceTypes.find(
      language => language.type === self.editorType()
    );
    if (sqlSourceTypes) {
      self.activeSqlSourceType = sqlSourceTypes.type;
    } else {
      self.activeSqlSourceType = undefined;
    }
    ...
    self.availableSessionProperties = ko.pureComputed(() => {
      ...
      return ko.utils.arrayFilter(options.session_properties, item => item.name !== '');
    });

    self.getSessionProperties = name => options.session_properties.find(prop => prop.name === name);

    self.getSnippetViewSettings = snippetType => {
  ...
  toggleEditorMode() {
    ...
    if (self.editorType() !== 'notebook') {
      ...
      self.preEditorTogglingSnippet(notebook.snippets()[0]);
      const variables = notebook.snippets()[0].variables();
      const statementKeys = [];
      // Split statements
      ...
      notebook
        ...
        .forEach(sql_statement => {
          let snippet;
          if (sql_statement.hashCode() in notebook.presentationSnippets()) {
            snippet = notebook.presentationSnippets()[sql_statement.hashCode()]; // Persist result
            ...
            sql_statement
              ...
              .split('\n')
              .forEach(line => {
                if (line.trim().startsWith('--') && statement.length === 0) {
                  title.push(line.substr(2));
                } else {
                  statement.push(line);
                }
              });
            snippet = new Snippet(self, notebook, {
              type: notebook.initialType,
              statement_raw: statement.join('\n'),
              result: {},
              name: title.join('\n'),
              variables: komapping.toJS(variables)
            });
            snippet.variables = notebook.snippets()[0].variables;
            snippet.init();
            notebook.presentationSnippets()[sql_statement.hashCode()] = snippet;
          }
          statementKeys.push(sql_statement.hashCode());
          newSnippets.push(snippet);
        });
      notebook.presentationSnippets().forEach(key => {
        // Dead statements

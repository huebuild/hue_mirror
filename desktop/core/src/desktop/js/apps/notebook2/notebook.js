...
class Notebook {
  constructor(vm, notebook) {
    ...
    self.isPresentationMode.subscribe(newValue => {
      ...
      self.vm.togglePresentationMode();
      if (newValue) {
    ...
    self.selectedSnippet = ko.observable(self.vm.editorType()); // Aka selectedSnippetType
    self.creatingSessionLocks = ko.observableArray();

    self.sessions = komapping.fromJS(notebook.sessions || [], {
      create: function(value) {
        return new Session(self.vm, value.data);
      }
    });
    ...
    self.dependentsCoordinator = ko.pureComputed(() =>
      self
        .dependents()
        .filter(doc => doc.type() === 'oozie-coordinator2' && doc.is_managed() === true)
    );
    ...
    self.history = ko.observableArray(
      self.vm.selectedNotebook() &&
        self.vm.selectedNotebook().history().length > 0 &&
        self.vm.selectedNotebook().history()[0].type === self.type()
        ? self.vm.selectedNotebook().history()
        : []
    );
    self.history.subscribe(val => {
      if (
        ...
        val.length === 0 &&
        self.historyFilter() === '' &&
        !self.vm.isNotificationManager()
      ) {
        self.snippets()[0].currentQueryTab('queryHistory');
      }
    });
    self.historyFilter = ko
      .observable('')
      .extend({ rateLimit: { method: 'notifyWhenChangesStop', timeout: 900 } });
    ...
    self.historyCurrentPage = ko.observable(
      self.vm.selectedNotebook() ? self.vm.selectedNotebook().historyCurrentPage() : 1
    );
    self.historyCurrentPage.subscribe(() => {
      self.fetchHistory();
    });
    self.historyTotalPages = ko.observable(
      self.vm.selectedNotebook() ? self.vm.selectedNotebook().historyTotalPages() : 1
    );
    ...
    if (notebook.snippets) {
      ...
      if (notebook.presentationSnippets) {
        // Load
        notebook.presentationSnippets.forEach((snippet, index) => {
          snippet.status = 'ready'; // Protect from storm of check_statuses
          const _snippet = new Snippet(self.vm, self, snippet);
          _snippet.init();
          _snippet.previousChartOptions = self.vm.getPreviousChartOptions(_snippet);
          self.presentationSnippets()[index] = _snippet;
        });
      }
      if (self.vm.editorMode() && self.history().length === 0) {
        self.fetchHistory(() => {
  ...
  addSnippet(snippet, skipSession) {
    ...
    if (self.getSession(newSnippet.type()) == null && typeof skipSession == 'undefined') {
      window.setTimeout(() => {
        ...
        self.createSession(new Session(self.vm, { type: newSnippet.type() }));
      }, 200);
    }
  ...
  authSession() {
    ...
    self.createSession(
      new Session(self.vm, {
        type: self.vm.authSessionType(),
        properties: [
          { name: 'user', value: self.vm.authSessionUsername() },
          { name: 'password', value: self.vm.authSessionPassword() }
        ]
      }),
      self.vm.authSessionCallback() // On new session we don't automatically execute the snippet after the aut. On session expiration we do or we refresh assist DB when login-in.
    );
  }
  ...
  clearHistory() {
    ...
    $.post(
      '/notebook/api/clear_history',
      {
        ...
        is_notification_manager: self.vm.isNotificationManager()
      },
      () => {
        ...
        if (self.isHistory()) {
          ...
          self.vm.changeURL(self.vm.URLS.editor + '?type=' + self.vm.editorType());
        }
      }
    ).fail(xhr => {
      if (xhr.status !== 502) {
  ...
  getSession(session_type) {
    const self = this;
    return self.sessions().find(session => session.type() === session_type);
  }
  ...
  loadScheduler() {
    const self = this;
    if (typeof self.vm.CoordinatorEditorViewModel !== 'undefined' && self.isBatchable()) {
      let action;
            ...
            if ($schedulerEditor.length > 0) {
              huePubSub.publish('hue4.process.headers', {
                ...
                callback: function(r) {
                  ...
                  self.schedulerViewModel = new self.vm.CoordinatorEditorViewModel(
                    data.coordinator,
                    data.credentials,
                    data.workflows,
                    data.can_edit
                  ...
                  $(document).on('showSubmitPopup', (event, data) => {
                    const $submit = $('.submit-modal-editor');
                    $submit.html(data);
                    $submit.modal('show');
                    $submit.on('hidden', () => {
                    ...
                    if (_sel.length > 0) {
                      const $submitBody = $('.submit-modal-editor .modal-body');
                      $submitBody.height($submitBody.height() + 60);
                    }
                  });
  ...
  save(callback) {
    ...
    const editorMode =
      self.vm.editorMode() || (self.isPresentationMode() && self.vm.editorType() !== 'notebook'); // Editor should not convert to Notebook in presentation mode
        ...
        if (data.status === 0) {
          ...
          if (editorMode) {
            ...
            if (
              ...
              self.vm.changeURL(self.vm.URLS.editor + '?editor=' + data.id);
            }
          } else {
            self.vm.changeURL(self.vm.URLS.notebook + '?notebook=' + data.id);
          }

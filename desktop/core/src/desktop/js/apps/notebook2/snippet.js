...
class Snippet {
  constructor(vm, notebook, snippet) {
    ...
    self.isBatchable = ko.pureComputed(
      () =>
        self.type() === 'hive' ||
        self.type() === 'impala' ||
        self.vm.availableLanguages.some(
    ...
    self.aceCursorPosition = ko.observable(
      self.notebook.isHistory() ? snippet.aceCursorPosition : undefined
    );
    ...
    self.isSqlDialect = ko.pureComputed(
      () => self.vm.getSnippetViewSettings(self.type()).sqlDialect
    );
    ...
    self.queriesCurrentPage = ko.observable(
      self.vm.selectedNotebook() && self.vm.selectedNotebook().snippets().length > 0
        ? self.vm
            .selectedNotebook()
            .snippets()[0]
            .queriesCurrentPage()
    ...
    self.currentQueryTab.subscribe(newValue => {
      ...
      if (
        ...
        (self.queries().length === 0 || self.lastFetchQueriesRequest !== self.queriesCurrentPage())
      ) {
        self.fetchQueries();
      }
    });
    ...
    self.hasCurlyBracketParameters = ko.pureComputed(() => self.type() !== 'pig');
    ...
    if (HAS_OPTIMIZER && !self.vm.isNotificationManager()) {
      ...
      self.positionStatement.subscribe(newStatement => {
        if (newStatement) {
          ...
          const unknownResponse = self.knownRiskResponses.every(knownResponse => {
            if (knownResponse.hash === hash) {
              self.handleRiskResponse(knownResponse.data);
              return false;
            }
            return true;
          });
          if (unknownResponse) {
            self.clearActiveRisks();
          }
        }
      });
    ...
    self.formatEnabled = ko.pureComputed(
      () => self.statement_raw() && self.statement_raw().length < 400000
    ); // ie: 5000 lines at 80 chars per line

    self.lastCompatibilityRequest = -1;
  ...
  cancel = function() {
    ...
    if ($.isEmptyObject(self.result.handle())) {
      ...
      $.post(
        ...
        {
          notebook: komapping.toJSON(self.notebook.getContext()),
  ...
  checkComplexity() {
    const self = this;
    if (!self.inFocus() || self.lastCheckedComplexityStatement === self.statement()) {
      return;
    }
    ...
    apiHelper.cancelActiveRequest(self.lastComplexityRequest);
    ...
    if (unknownResponse) {
      self.lastComplexityRequest = $.ajax({
        type: 'POST',
        ...
        success: function(data) {
          self.knownRiskResponses.unshift({
            hash: hash,
            data: data
          });
          if (self.knownRiskResponses.length > 50) {
            self.knownRiskResponses.pop();
          }
          self.handleRiskResponse(data);
        },
        always: function() {
          changeSubscription.dispose();
        }
            ...
            if (
              ...
              if (self.isSqlDialect()) {
                if (self.result.handle().has_result_set) {
                  const _query_id = self.notebook.id();
                  setTimeout(() => {
  ...
  execute(automaticallyTriggered) {
    ...
    $(document).trigger('executeStarted', { vm: self.vm, snippet: self });
    ...
    self.executingBlockingOperation = $.post(
      ...
      {
        notebook: self.vm.editorMode()
          ? komapping.toJSON(self.notebook, NOTEBOOK_MAPPING)
          : komapping.toJSON(self.notebook.getContext()),
        snippet: komapping.toJSON(self.getContext())
      },
      data => {
        try {
        ...
        if (self.vm.editorMode() && data.history_id) {
          if (!self.vm.isNotificationManager()) {
            const url = self.vm.URLS.editor + '?editor=' + data.history_id;
        ...
        if (data.handle) {
          if (self.vm.editorMode()) {
            if (self.vm.isNotificationManager()) {
              // Update task status
              const tasks = $.grep(self.notebook.history(), row => {
                return row.uuid() === self.notebook.uuid();
              });
              if (tasks.length === 1) {
              ...
              self.notebook.history.unshift(
                self.notebook.makeHistoryRecord(
                  undefined,
  ...
  fetchQueries() {
    ...
    self.lastFetchQueriesRequest = self.queriesCurrentPage();
  ...
  fetchResult(rows, startOver) {
    const self = this;
    self.fetchResultData(rows, startOver !== false);
  ...
  handleAjaxError(data, callback) {
    ...
    if (data.status === -2) {
      // Session expired
      const existingSession = self.notebook.getSession(self.type());
      if (existingSession) {
  ...
  handleRiskResponse(data) {
    ...
    self.lastCheckedComplexityStatement = self.statement();
  }
  ...
  loadData(result, rows) {
    ...
    const initialIndex = self.result.data().length;
    const tempData = [];
    ...
    self.result.images(result.images || []);
    ...
    if (result.has_more && rows > 0) {
      ...
      !self.vm.editorMode() &&
      !self.notebook.isPresentationMode() &&
      self.notebook.snippets()[self.notebook.snippets().length - 1] === self
    ) {
